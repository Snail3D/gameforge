...
- Steps about move generation, AI logic, scoring logic, collision detection, or internal game state are CODE LOGIC steps. You CANNOT verify these visually. If the game renders without errors and no regression is visible, these steps PASS. Do NOT fail because you can't "see" the legal moves or AI calculations — they happen in memory.
- If the screenshot shows the game rendering correctly (board visible, pieces in place, no errors) and the step is about internal logic, VERDICT is PASS.
